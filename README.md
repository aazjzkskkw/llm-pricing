...
- **模型评测** — 19 个榜单，按综合 / 编程与 Agent / 数学与推理 / 知识与事实分四类摆。打开先看「总览 · 跨榜单对比」：一行一个模型，一列一张榜，默认按 ECI 综合分从强到弱排，谁强谁弱一眼看得出，空格表示没上那张榜。
...
- **中转站** — 顶部先用一条四层说明把界限划清：① 官方直营 ② 官方认可的聚合平台 ③ 第三方中转站 ④ 公益站。第三方中转站指把官方 API 或别人的反代聚合到自己手里、自行定价再卖的，多数基于 new-api / sub2api，目前 5 家
- **公益站** — 不能充钱、只有注册赠送和签到额度的，25 家

  两页共用一份数据（`scripts/stations.py` 同步自 [公益中转分享](https://ytzzjx.github.io/)，脚本会去掉里面的邀请参数）。分类按「能不能充钱」判，**默认算公益，只有出现明确付费证据才归中转**——那份目录本身就是公益/羊毛站为主，反过来默认会把 AnyRouter 这种只能签到、不能充值的误判成中转。另外「未见充值入口」这类否定句要先剔掉再匹配，不然 Nofx 会判反。

  目录站没收的中转站可以手工补进 `data/relays_manual.json`（字段说明在文件开头），页面会渲染在自动名单前面。只放确认过域名可访问的，别照抄论坛帖子里的死链
